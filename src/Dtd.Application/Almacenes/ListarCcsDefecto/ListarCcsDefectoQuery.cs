...
/// <summary>Lista los CCs por defecto de una tupla (empresa, almacén, agencia) para que el front los
/// auto-adjunte al generar un documento para esa tupla. El back no los auto-adjunta — los añade el
/// front vía <c>POST /documentos/{id}/ccs</c>. Espejo exacto de <c>ListarAgenciaBasesDefectoQuery</c>.</summary>
public sealed record ListarCcsDefectoQuery(string Empresa, string AlmacenCodigo, string AgenciaCodigo)
    : IRequest<ErrorOr<IReadOnlyList<CcCatalogoDto>>>;

internal sealed class ListarCcsDefectoQueryHandler
    : IRequestHandler<ListarCcsDefectoQuery, ErrorOr<IReadOnlyList<CcCatalogoDto>>>
{
    ...
    public async Task<ErrorOr<IReadOnlyList<CcCatalogoDto>>> Handle(
        ListarCcsDefectoQuery request, CancellationToken cancellationToken)
    {
