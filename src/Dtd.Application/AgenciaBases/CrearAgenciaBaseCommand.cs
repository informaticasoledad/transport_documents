...
internal sealed class CrearAgenciaBaseCommandValidator : AbstractValidator<CrearAgenciaBaseCommand>
{
    public CrearAgenciaBaseCommandValidator()
    {
        ...
        RuleFor(x => x).Must(x => CanalContactoCoherente(x.Canal, x.Email, x.Movil))
            .WithMessage(x => $"El canal '{x.Canal}' requiere un contacto coherente (email->Email; sms/whatsapp->Movil).");
    }

    internal static bool CanalContactoCoherente(string canal, string? email, string? movil)
    {
        var c = (canal ?? string.Empty).Trim().ToLowerInvariant();
        if (c == Canal.Email) return !string.IsNullOrWhiteSpace(email);
        if (c == Canal.Sms || c == Canal.Whatsapp) return !string.IsNullOrWhiteSpace(movil);
        return false;
    }
}

internal sealed class CrearAgenciaBaseCommandHandler
    : IRequestHandler<CrearAgenciaBaseCommand, ErrorOr<AgenciaBaseCatalogoDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var existente = await _agenciaBaseRepository.GetByEmpresaYCodigoAsync(
            empresa,
            request.Codigo,
            cancellationToken);
        ...
        if (existente is not null)
        {
            return Error.Conflict(
                ...
                $"Ya existe un agenciaBase con codigo '{request.Codigo}' en la empresa '{empresa}'.");
        }
        ...
        await _agenciaBaseRepository.AddAsync(agenciaBase, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        return ToDto(agenciaBase);
    }

    internal static AgenciaBaseCatalogoDto ToDto(AgenciaBase c) => new()
    {
        Id = c.Id,
        Codigo = c.Codigo,
        Nombre = c.Nombre,
